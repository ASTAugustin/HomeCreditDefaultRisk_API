...
class appTrainTransformer_Lefei(BaseEstimator, TransformerMixin):
    def __init__(self):
        return None
    ...
    def transform(self, X, y=None):
        ...
        X['ID_TO_BIRTH_RATIO'] = X['DAYS_ID_PUBLISH'] / X['DAYS_BIRTH']
        X['CAR_TO_BIRTH_RATIO'] = X['OWN_CAR_AGE'] / X['DAYS_BIRTH']
        X['CAR_TO_EMPLOYED_RATIO'] = X['OWN_CAR_AGE'] / X['DAYS_EMPLOYED']
        X['PHONE_TO_BIRTH_RATIO'] = X['DAYS_LAST_PHONE_CHANGE'] / X['DAYS_BIRTH']

        X.replace([np.inf, -np.inf], np.nan, inplace=True)
        return X

class dropTransformer_Lefei(BaseEstimator, TransformerMixin):
...
class bureauTransformer_Lefei(BaseEstimator, TransformerMixin):
    def __init__(self):
        return None
    ...
    def transform(self, bureauinfo, bbinfo, y=None):
        ...
        bureau_copy['CREDIT_DURATION'] = -bureau_copy['DAYS_CREDIT'] + bureau_copy['DAYS_CREDIT_ENDDATE']
        bureau_copy['ENDDATE_DIF'] = bureau_copy['DAYS_CREDIT_ENDDATE'] - bureau_copy['DAYS_ENDDATE_FACT']
        # Credit to debt ratio and difference
        bureau_copy['DEBT_PERCENTAGE'] = bureau_copy['AMT_CREDIT_SUM'] / bureau_copy['AMT_CREDIT_SUM_DEBT']
        bureau_copy['DEBT_CREDIT_DIFF'] = bureau_copy['AMT_CREDIT_SUM'] - bureau_copy['AMT_CREDIT_SUM_DEBT']
        bureau_copy['CREDIT_TO_ANNUITY_RATIO'] = bureau_copy['AMT_CREDIT_SUM'] / bureau_copy['AMT_ANNUITY']
...
class previousTransformer_Lefei(BaseEstimator, TransformerMixin):
    def __init__(self):
        return None
...
class posTransformer_Lefei(BaseEstimator, TransformerMixin):
    def __init__(self):
        return None
...
class installmentsTransformer_Lefei(BaseEstimator, TransformerMixin):
    def __init__(self):
        return None
...
class ccTransformer_Lefei(BaseEstimator, TransformerMixin):
    def __init__(self):
        return None
...
class dropImportanceTransformer_Lefei(BaseEstimator, TransformerMixin):
...
class kfoldlgb_Lefei(BaseEstimator, TransformerMixin):
    def __init__(self, k):
        self.k = k
        self.clf = []
        return None
    ...
    def fit(self, X, y=None):
        ...
        feature['feature_Lefei'] = X_feature
        feature.to_csv('feature_Lefei.csv', index = False)
        return self
